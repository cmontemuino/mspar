...
int gensam( char **list, double *pprobss, double *ptmrca, double *pttot ){
	...
	nsinv = 1./nsites;
    // @TODO: myRank is already global, why do we need to redefine it here?
	int myRank;
	MPI_Comm_rank(MPI_COMM_WORLD, &myRank);
...
/*
 * Esta función se encarga de asignar trabajo a un worker. La asignación de trabajo consiste en
 * enviar el número de muestras a generar.
 *
 * @param workersActivity estado de actividad de los workers (0=ocioso; 1=ocupado)
 * @param worker el índice del worker al que se le asignará trabajo
 * @param samples la cantidad de muestras que ha de generar el worker
 */
...
// PODEMOS UTILIZAR MPI_Sendv??
  workersActivity[worker]=1;
}

// **************************************  //
// LÓGICA DE LOS WORKERS
// **************************************  //

/*
 * Lógica de procesamiento del WORKER
 *
 * @param myRank rango del worker
 * @param samples el número de muestras a generar
 *
 * @return una cadena con los resultados de la genealogía
 */
char* workerProcessingLogic(int myRank, int samples) {
  int gensam(char **gametes, double *probss, double *ptmrca, double *pttot);
  int doCalculateWorkerResultLength(int segsites);
  void doPrintWorkerResultHeader(int segsites, char *results);
  ...
  bytes = doCalculateWorkerResultLength(segsites);
  results = (char *) malloc(bytes*sizeof(char));
  doPrintWorkerResultHeader(segsites, results);
...
/**
 * Calcula el largo en bytes que tendrá la cadena de resultados de un worker en base al número de segsites.
 * Este valor es luego utilizado en el malloc() que se hace para reservar la memoria para
 * la salida de datos del worker.
 */
int doCalculateWorkerResultLength(int segsites){
  int result = 4; // salto de línea, los '//' para indicar los tbs y otro salto de línea
  int i;
  char *integerString;
  
  // Línea de lo segsites
  result += 11; // palabra "segsites: " + salto de línea
  asprintf(&integerString, "%i", segsites);
  result += strlen(integerString);
  
  // Línea de las positions
  result += 12; // palabra "positions: " + salto de línea
  result += segsites * 7; // cada "position" tiene 6 caracteres + 1 espacio
  
  // Líneas con los gametos
  result += pars.cp.nsam*(segsites + 1)+1; // "1" por cada salto de línea; se agrega un 1 al final como workaround, sino da problemas de malloc con más de 300 muestras
   
  return result;
}

/*
 * Prepara la impresión de la primera parte de la salida del worker:
 *    \n
...
void doPrintWorkerResultHeader(int segsites, char *results){
  ...
  if( (segsites > 0 ) || ( pars.mp.theta > 0.0 ) ) {
    ...
    strcat(results, tempString); // VER DE UTILIZAR ALTERNATIVA A strcat
  }
}

/*
 * Prepara la impresión de los positions de la salida del worker:
...
void doPrintWorkerResultPositions(int segsites, char *results){
  ...
  for(i=0; i<segsites; i++){
    asprintf(&tempString, "%6.4lf ", posit[i]);
...
/*
 * Prepara la impresión de los gametos en la salida del worker.
 */
void doPrintWorkerResultGametes(char **gametes, char *results){
  int i;
  char *tempString;
...
/*
 * Esta función escucha mensajes del Master y retorna la cantidad de muestras a ser generadas que éste le pasa.
 *
 * @return la cantidad de muestras a ser generadas
 */
int receiveWorkRequest(){
  int samples;
  MPI_Status status;
...
/*
 * Esta función se encarga de enviar los resultados de un worker al master para que los termine de procesar.
 *
 * @param myRank rango del worker
 * @param results resultados a enviar
 * 
 */
void sendResultsToMasterProcess(int myRank, char* results) {
...
/* INICIALIZACIÓN DE LA PARTE ALEATORIA CUANDO SE EJECUTA EN PARALELO. */
unsigned short * parallelSeed(unsigned short *seedv){
